			...
			<ul class="tab_wrap outer">
				<li class="active">
					<div class="tab_con ty2">
						...
						<div align="right">
							<input style="cursor:pointer; border-color: transparent; background-color: red; color: white; width: 90px; height: 35px;" type="button" value="추가하기" onclick="location.href='${root }index?formpath=timeInfoInsert'">
						</div>
						<table class="tb_acc_wrap" summary="FAQ 표입니다. 구분, 질문 순서로 행이 구성되어 있습니다.">
							...
							<thead>
								<tr>
									...
									<th scope="col" id="thead1">영화관 번호</th>
							...
							<c:forEach var="info" items="${timeInfoList }">
								<tr class="acc_head" aria-expanded="false" role="button" id="tr_180">
									...
									<td id="row0" headers="thead0">${info.getHallNum() }</td>
									<td id="row0" headers="thead0">${info.getTicketDate() }</td>
									<td id="row0" headers="thead0">${info.getStartTime() }</td>
									<td id="row0" headers="thead0" onclick="location.href='timeInfoDeleteProc?num=${info.getTimeInfoNum() }'">삭제하기</td>
								</tr>
							</c:forEach>
							</tbody>
						</table>
